...
class PatientClient:
    def __init__(self, host='127.0.0.1', port=5000, patient_id="Patient_1"):
        ...
        # Domain parameters (generated by the client)
        self.prime = None
        self.g = None
    ...
    def exchange_public_key(self):
        # Generate domain parameters and key pair.
    ...
    def authenticate(self):
        """
        1. Generate session key, encrypt it for the doctor using ElGamal.
        2. Send (c1, c2) to the server along with signatures.
        3. Receive re-encrypted session key from server, verify it.
        """
        try:
            ...
            # ElGamal encryption of the session key -> (c1, c2)
            ...
            # Step 3: re-encrypted session key
            ...
            # Decrypt re-encrypted session key
    ...
    def decrypt_with_aes(self, encrypted_message, key):
        """
        For AES-CBC, the first block_size (16) bytes of encrypted_message is the IV.
    ...
    def listen_for_broadcasts(self):
        """
        Continuously reads messages (opcodes) from the server:
        - opcode 30: new group key in AES-CBC
        - opcode 40: broadcast message in AES-CBC
        - opcode 80: blocked
        - opcode 60: server shutdown
        """
        try:
            while True:
                data = self.client.recv(4096)
                ...
                if "opcode" in msg:
                    if msg["opcode"] == 30:
                        ...
                        # Received broadcast message, use self.group_key
                        encrypted_message = msg["encrypted_message"]
                        if not self.group_key:
                            print("[INFO] No group key available yet.")
                            continue
                        key_for_group = self.group_key[:32]  # first 16 bytes in hex form
